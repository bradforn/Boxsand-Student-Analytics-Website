...
class course(models.Model):
    ...
    def awesome_plot(self,plot, search_by = None):
        ...
        #folder = "Graphs/"
        file_name = self.name + "_scatter_" + plot + ".html"
    ...
    def agg_scatter_plot(self,plot, search_by = None):
        ...
        layout = go.Layout(
                        title= "<b>" + self.name + "</b>" + "<br> P-value: "+ '%.2E'%Decimal(p_value)+ "<br> Slope: "+ '%.2E'%Decimal(slope),
...
class student(models.Model):              #defining student class
...
def retrieve_data():
    with open("{% static 'polls/style.css' %}" , "rb") as f:
        dump = pickle.load(f)
    return dump
